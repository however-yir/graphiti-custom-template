...
class QueueService:
    ...
    def get_total_queue_size(self) -> int:
        """Get total queued tasks across all groups."""
        return sum(queue.qsize() for queue in self._episode_queues.values())

    def get_active_worker_count(self) -> int:
        """Get number of currently active queue workers."""
        return sum(1 for running in self._queue_workers.values() if running)

    def get_group_stats(self) -> dict[str, dict[str, int | bool]]:
        """Return per-group queue and worker status for diagnostics."""
        stats: dict[str, dict[str, int | bool]] = {}
        for group_id, queue in self._episode_queues.items():
            stats[group_id] = {
                'queue_size': queue.qsize(),
                'worker_running': self._queue_workers.get(group_id, False),
            }
        return stats
