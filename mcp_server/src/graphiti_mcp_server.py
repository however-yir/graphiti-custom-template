#!/usr/bin/env python3
"""Graphiti Custom MCP Server - Exposes temporal graph memory through MCP."""
...
Graphiti Custom is a memory service for AI agents built on a knowledge graph. It performs well
with dynamic data such as user interactions, changing enterprise data, and external information.

Graphiti Custom transforms information into a richly connected knowledge network, allowing you to 
...
# MCP server instance
mcp = FastMCP('Graphiti Custom Agent Memory', instructions=GRAPHITI_MCP_INSTRUCTIONS)
...
def _queue_health_snapshot() -> dict[str, Any]:
    ...
    return {
        'initialized': True,
        'total_queue_size': queue_service.get_total_queue_size(),
        'active_workers': queue_service.get_active_worker_count(),
        'groups': queue_service.get_group_stats(),
    }
...
async def get_queue_status() -> QueueStatusResponse:
    """Get queue backlog and worker state for episode ingestion."""
    if queue_service is None:
...
async def health_check(request) -> JSONResponse:
    ...
    db_provider = (
        graphiti_service.config.database.provider
        if graphiti_service is not None and graphiti_service.config is not None
        else 'unknown'
    )
    return JSONResponse(
        {
            'status': 'healthy',
            'service': 'graphiti-custom-mcp',
...
async def liveness_check(request) -> JSONResponse:
...
async def readiness_check(request) -> JSONResponse:
    ...
    if graphiti_service is None:
        return JSONResponse(
            {
                'status': 'not_ready',
                'reason': 'graphiti_service_not_initialized',
                'uptime_seconds': _uptime_seconds(),
            },
            status_code=503,
        )
        ...
        return JSONResponse(
            {
                'status': 'ready',
                'service': 'graphiti-custom-mcp',
                'database_provider': graphiti_service.config.database.provider,
                'uptime_seconds': _uptime_seconds(),
                'queue': _queue_health_snapshot(),
            }
        )
...
async def metrics(request) -> PlainTextResponse:
    ...
    queue_snapshot = _queue_health_snapshot()
    ready = 1 if graphiti_service is not None else 0
    database_provider = (
        graphiti_service.config.database.provider if graphiti_service is not None else 'unknown'
    )

    lines = [
        '# HELP graphiti_custom_mcp_up Service readiness flag (1 ready, 0 not ready)',
        '# TYPE graphiti_custom_mcp_up gauge',
        f'graphiti_custom_mcp_up {ready}',
        '# HELP graphiti_custom_mcp_uptime_seconds Process uptime in seconds',
        '# TYPE graphiti_custom_mcp_uptime_seconds gauge',
        f'graphiti_custom_mcp_uptime_seconds {_uptime_seconds()}',
        '# HELP graphiti_custom_mcp_queue_total Pending episode tasks across all groups',
        '# TYPE graphiti_custom_mcp_queue_total gauge',
        f'graphiti_custom_mcp_queue_total {queue_snapshot["total_queue_size"]}',
        '# HELP graphiti_custom_mcp_queue_workers_active Number of active queue workers',
        '# TYPE graphiti_custom_mcp_queue_workers_active gauge',
        f'graphiti_custom_mcp_queue_workers_active {queue_snapshot["active_workers"]}',
        '# HELP graphiti_custom_mcp_database_info Database provider info',
        '# TYPE graphiti_custom_mcp_database_info gauge',
        f'graphiti_custom_mcp_database_info{{provider="{database_provider}"}} 1',
    ]

    for group_id, group_stat in queue_snapshot.get('groups', {}).items():
        lines.append(
            f'graphiti_custom_mcp_queue_group_size{{group_id="{group_id}"}} {group_stat["queue_size"]}'
        )
        worker_running = 1 if group_stat['worker_running'] else 0
        lines.append(
            f'graphiti_custom_mcp_queue_group_worker_running{{group_id="{group_id}"}} {worker_running}'
        )
